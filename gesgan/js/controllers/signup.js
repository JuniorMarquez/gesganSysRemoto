...
app.controller('SignupFormController', ['$scope', '$http', '$state','MyService', function($scope, $http, $state,MyService) {
    ...
    $scope.ok = function(user) {
      ...
      // Try to create
      $http.post('http://52.33.127.122:1337/usergesgan/', {nombre: $scope.user.nombre, email: $scope.user.email, password: $scope.user.password});
      $scope.entrar();
   };

     
    $scope.entrar = function(user) {
    $http.get('http://52.33.127.122:1337/usergesgan/?email=' +$scope.user.email).success(function(respuesta){
